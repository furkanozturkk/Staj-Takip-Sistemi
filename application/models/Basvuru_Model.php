...
class Basvuru_Model extends CI_Model{
	...
	public function kabulgungetir($no){
	 ...
	 public function tum_basvurular($id)
	 {
		$result = $this->db->query('SELECT ogr.ogrenci_no,ogr.ad_soyad,sbilgi.id sbilgi_id,sbilgi.baslama_tarihi,sbilgi.bitis_tarihi,sbilgi.staj_durum,sbilgi.staj_gun,syeri.stajyeri_ad,syeri.stajyeri_faaliyetAlan,syeri.id, syeri.stajyeri_mail,syeri.stajyeri_telefon,syeri.stajyeri_fax,syeri.stajyeri_adres,syeri.stajyeri_website,syeri.yetkili_adsoyad,syeri.yetkili_unvan,syeri.yetkili_mail,syeri.durum FROM tbl_stajbilgileri sbilgi 
		INNER JOIN tbl_stajyeri syeri on sbilgi.stajyeri_id=syeri.id INNER JOIN tbl_ogrenciler ogr on ogr.ogrenci_no = sbilgi.ogrenci_no 
		where sbilgi.staj_durum = 0')->result();
		return $result;
	 } 
	 public function onaylanmis_basvurular($id)
	 {
		...
		INNER JOIN tbl_ogrenciler ogr on ogr.ogrenci_no = sbilgi.ogrenci_no where staj_durum=1 and akademisyen_no='.$id.'')->result();
		return $result;
	 } 
	 public function reddedilmis_basvurular()
	 {
		$result = $this->db->query('SELECT ogr.ogrenci_no,ogr.ad_soyad,sbilgi.id sbilgi_id,sbilgi.baslama_tarihi,sbilgi.bitis_tarihi,sbilgi.staj_durum,sbilgi.staj_gun,syeri.stajyeri_ad,syeri.stajyeri_faaliyetAlan,syeri.id,
		syeri.stajyeri_mail,syeri.stajyeri_telefon,syeri.stajyeri_fax,syeri.stajyeri_adres,syeri.stajyeri_website,syeri.yetkili_adsoyad,syeri.yetkili_unvan,syeri.yetkili_mail,syeri.durum
		FROM tbl_stajbilgileri sbilgi INNER JOIN tbl_stajyeri syeri on sbilgi.stajyeri_id=syeri.id 
		INNER JOIN tbl_ogrenciler ogr on ogr.ogrenci_no = sbilgi.ogrenci_no where staj_durum=2')->result();
		return $result;
	 } 
